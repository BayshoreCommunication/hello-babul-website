...
import { Loader2, Mail, Notebook, User } from "lucide-react";
import { useEffect, useState } from "react";
import { LuEye } from "react-icons/lu";
import { RiDeleteBinLine } from "react-icons/ri";
import DeleteModal from "./DeleteModal";
import { deleteVolunteer, getVolunteers } from "@/app/actions/submissions";
...
const VolunteerApplication = () => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const response = await deleteVolunteer(selectedItem._id);
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item._id} className="border-b border-gray-100">
                  ...
                  <td className="py-4 text-black">{item.education}</td>
                  <td className="py-4">
                    <span
                  ...
                  <td className="py-4 text-center space-x-3">
                    <button
                      ...
                      onClick={() => {
                        ...
                        window.location.href = `/dashboard/volunteer-application/${item._id}`;
                      }}
                    >
                      <LuEye size={16} />
                    </button>
