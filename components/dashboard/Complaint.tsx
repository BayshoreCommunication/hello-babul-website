    ...
    async function fetchData() {
      try {
        ...
        if (mounted && response.success) {
          ...
          const generalSuggestions = response.data.filter(
            (item: Suggestion) => item.typeOfSuggest === "general"
          );
          setData(generalSuggestions);
        }
      } catch (err) {
        console.error("Failed to fetch general suggestions:", err);
        if (mounted) {
            ...
            <tbody>
              {data.map((item) => (
                <tr key={item._id} className="border-b border-gray-100">
                  ...
                  <td className="py-4 text-black">সাধারণ</td>
                  <td className="py-4">
                    <span
