  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const res = await fetch(
        "https://hello-babul-backend.vercel.app/api/volunteers",
        {
          method: "POST",
          body: data,
        }
      ...
      {open && (
        ...
        >
          <form
            ...
            className="px-6 py-3 flex flex-col gap-6 text-white"
          >
            {fields.map((field, i) => (
              <div
                key={i}
                className="flex flex-col md:flex-row gap-4 items-center relative w-full"
              >
            ...
            <div className="flex flex-col md:flex-row gap-4 items-center">
              <label className="md:w-1/4 text-lg shrink-0">ছবি</label>
